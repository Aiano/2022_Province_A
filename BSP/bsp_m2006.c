...
void M2006_Process()
{
	...
	/************* 单环 *************/
	pid_speed1.target_val = -5000;
	M2006_Get_Feedback(0x201, CANRxData);
	GetSpeed = M2006_Get_Speed(1);
	M2006_Set_I(speed_pid_realize(&pid_speed1, GetSpeed), 1); // 电流设置
}
...
uint8_t M2006_Set_I(int target_i, uint8_t motor_id)
{
	...
	if (motor_id >= 1 && motor_id <= 8)
	{
		...
		CAN_SendData(M2006_CAN, M2006_Sendbuf, 0x200);
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *canHandle)
{
	...
	HAL_CAN_GetRxMessage(canHandle, CAN_RX_FIFO0, &RxMessage, CANRxData);
